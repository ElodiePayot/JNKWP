...
hide=true;$('body').on('touchstart',function(){if(hide)$('#h1Icon').removeClass('active');hide=true;});$('body').on('touchstart','#h1Icon',function(){var self=$(this);if(self.hasClass('active')){$('#h1Icon').removeClass('active');return false;}
$('#h1Icon').removeClass('active');self.toggleClass('active');hide=false;});(function($){$.confetti=new function(){var canvas;var ctx;var W;var H;var mp=100;var particles=[];var angle=0;var tiltAngle=0;var confettiActive=true;var animationComplete=true;var deactivationTimerHandler;var reactivationTimerHandler;var animationHandler;var particleColors={colorOptions:['#FBDE46','#508BFC','#024F65','#34A853','#FF4545'],colorIndex:0,colorIncrementer:0,colorThreshold:10,getColor:function(){if(this.colorIncrementer>=10){this.colorIncrementer=0;this.colorIndex++;if(this.colorIndex>=this.colorOptions.length){this.colorIndex=0;}}
...
function InitializeButton(){$('#restartConfetti').click(RestartConfetti);}
function SetGlobals(){$('body').append('<canvas id="confettiCanvas" style="position:absolute;top:0;left:0;display:none;z-index:99;pointer-events:none;"></canvas>');canvas=document.getElementById("confettiCanvas");ctx=canvas.getContext("2d");W=window.innerWidth;H=window.innerHeight;canvas.width=W;canvas.height=H;}
...
function CheckForReposition(particle,index){if((particle.x>W+20||particle.x<-20||particle.y>H)&&confettiActive){if(index%5>0||index%2==0)
{repositionParticle(particle,Math.random()*W,-10,Math.floor(Math.random()*10)-10);}else{if(Math.sin(angle)>0){repositionParticle(particle,-5,Math.random()*H,Math.floor(Math.random()*10)-10);}else{repositionParticle(particle,W+5,Math.random()*H,Math.floor(Math.random()*10)-10);}}}}
...
function RestartConfetti(){reactivationTimerHandler=setTimeout(function(){confettiActive=true;animationComplete=false;InitializeConfetti();},100);setTimeout(function(){ClearTimers();DeactivateConfetti();},2000);}
